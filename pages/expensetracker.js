...
const ExpenseTracker = () => {
  ...
  useEffect(() => {
    ...
    const fuelrate = async (location, fuelType) => {
      try {
        const response = await fetch(
          `https://mocki.io/v1/a04a6bbb-565a-47aa-b20a-050a6c822958`,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch fuel rates");
        }

        const data = await response.json();

        setData(data.fuel_prices.Maharashtra.petrol);
    ...
    fuelrate("Maharashtra", "petrol").then((rate) => {
      console.log("Fuel Price:", rate);
    });
